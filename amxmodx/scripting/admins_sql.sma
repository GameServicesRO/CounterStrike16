...
public adminSql()
{
	...
	if (equali(type, "sqlite"))
	{
		...
		SQL_QueryAndIgnore(sql, "CREATE TABLE IF NOT EXISTS `%s_groups` (`id` INT(11) NOT NULL AUTO_INCREMENT, `name` VARCHAR( 64 ) NOT NULL, `flags` VARCHAR( 64 ) NOT NULL, `additional_properties` JSON NULL, PRIMARY KEY(id)) COMMENT = 'GameServices Admins Groups';", table)
		query = SQL_PrepareQuery(sql,"SELECT `auth`,`password`,`access`,`flags` FROM `%s`", table)
	}
