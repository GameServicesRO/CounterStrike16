...
// if you want to use threaded queries for SQL, uncomment the line below
#define THREAD_QUERIES
...
{
	new szData[4][128];
	get_cvar_string("amx_sql_host", szData[0], charsmax(szData[]));
	get_cvar_string("amx_sql_user", szData[1], charsmax(szData[]));
	get_cvar_string("amx_sql_pass", szData[2], charsmax(szData[]));
	get_cvar_string("amx_sql_db", szData[3], charsmax(szData[]));
	g_sql_tuple = SQL_MakeDbTuple(szData[0], szData[1], szData[2], szData[3], 20);
	...
	new Handle:connection = SQL_Connect(g_sql_tuple, errcode, error, sizeof(error) - 1);
	
	if( connection == Empty_Handle )
	{
		set_fail_state("Unable to connect to database.");
	}
	
	new Handle:result = SQL_PrepareQuery(connection, "%s", query);
	
	if( !SQL_Execute(result) )
	{
		SQL_QueryError(result, error, sizeof(error) - 1);
		log_amx("Failed to create table for bans: %s", error);
	}
...
public CmdBan(client, level, cid)
{
	...
	AddBan(target_name, target_authid, arg, length, unban_time, admin_name, admin_authid, g_szServerName, get_systime());
...
public CmdBanIp(client, level, cid)
{
	...
	AddBan(target_name, target_ip, arg, length, unban_time, admin_name, admin_authid, g_szServerName, get_systime());
...
public CmdAddBan(client, level, cid)
{
	...
	AddBan(target_name, target_authid, reason, length, unban_time, admin_name, admin_authid, g_szServerName, get_systime());
...
public FwdThink(entity)
{
	...
	#if defined USING_SQL && defined THREAD_QUERIES
...
AddBan(const target_name[], const target_steamid[], const reason[], const length, const unban_time[], const admin_name[], const admin_steamid[], const server_name[], const current_time)
{
	#if MAX_BANS > 0
	...
	static query[512];
	formatex(query, sizeof(query) - 1,\
		"INSERT INTO %s (%s, %s, %s, %s, %s, %s, %s, %s, %s) VALUES ('%s', '%s', '%i', '%s', '%s', '%s', '%s', '%s', '%i');",\
		TABLE_NAME, KEY_NAME, KEY_STEAMID, KEY_BANLENGTH, KEY_UNBANTIME, KEY_REASON, KEY_ADMIN_NAME, KEY_ADMIN_STEAMID, KEY_SERVER_NAME, KEY_BAN_TIME,\
		target_name2, target_steamid, length, unban_time, reason2, admin_name2, admin_steamid, server_name, current_time
		);
	...
	{
		SQL_QueryError(result, error, sizeof(error) - 1);
		log_amx("Failed to add ban: %s", error);
	}
...
public QueryLoadBans(failstate, Handle:query, error[], errcode, data[], datasize, Float:queuetime)
{
	...
	{
		SQL_GetResults(query);
	}
}
#endif

SQL_GetResults(Handle:query)
{
	if( SQL_NumResults(query) )
	{
		...
		{
			#if MAX_BANS <= 0
			SQL_ReadResult(query, 1, data[bd_name], sizeof(data[bd_name]) - 1);
			SQL_ReadResult(query, 2, data[bd_steamid], sizeof(data[bd_steamid]) - 1);
			data[bd_banlength] = SQL_ReadResult(query, 3);
			SQL_ReadResult(query, 4, data[bd_unbantime], sizeof(data[bd_unbantime]) - 1);
			SQL_ReadResult(query, 5, data[bd_reason], sizeof(data[bd_reason]) - 1);
			SQL_ReadResult(query, 6, data[bd_admin_name], sizeof(data[bd_admin_name]) - 1);
			SQL_ReadResult(query, 7, data[bd_admin_steamid], sizeof(data[bd_admin_steamid]) - 1);
			
			ArrayPushArray(g_array, data);
			TrieSetCell(g_trie, data[bd_steamid], g_total_bans);
			#else
			SQL_ReadResult(query, 1, g_names[g_total_bans], sizeof(g_names[]) - 1);
			SQL_ReadResult(query, 2, g_steamids[g_total_bans], sizeof(g_steamids[]) - 1);
			g_banlengths[g_total_bans] = SQL_ReadResult(query, 3);
			SQL_ReadResult(query, 4, g_unbantimes[g_total_bans], sizeof(g_unbantimes[]) - 1);
			SQL_ReadResult(query, 5, g_reasons[g_total_bans], sizeof(g_reasons[]) - 1);
			SQL_ReadResult(query, 6, g_admin_names[g_total_bans], sizeof(g_admin_names[]) - 1);
			SQL_ReadResult(query, 7, g_admin_steamids[g_total_bans], sizeof(g_admin_steamids[]) - 1);
			#endif
...
RemoveBan(remove)
{
	...
	{
		SQL_QueryError(result, error, sizeof(error) - 1);
		log_amx("Failed to remove ban: %s", error);
	}
...
public LoadBans()
{
	...
	{
		SQL_QueryError(result, error, sizeof(error) - 1);
		log_amx("Failed to load bans: %s", error);
	}
	else
	{
		SQL_GetResults(result);
	}
